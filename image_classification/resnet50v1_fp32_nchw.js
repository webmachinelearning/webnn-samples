...
// ResNet50 V1 fp32 model with 'nchw' input layout
export class ResNet50V1Fp32Nchw {
  constructor() {
    this.context_ = null;
    this.builder_ = null;
    this.graph_ = null;
    ...
    this.weightsUrl_ = weightsOrigin() +
      '/test-data/models/resnet50v1_fp32_nchw/weights/';
    this.inputOptions = {
  ...
  async buildBottleneck_(
    ...
    let residual = input;
    // Bottleneck: 1x1 (stride 1), 3x3 (stride S when downsampling), 1x1 — matches ONNX/PyTorch.
    const conv1 = this.buildConv_(input, nameIndex,
        stageName, true, {});
    const conv2Opts = {padding: [1, 1, 1, 1]};
    if (downsample && stride > 1) {
      conv2Opts.strides = [stride, stride];
    }
    const conv2 = this.buildConv_(conv1, parseInt(nameIndex) + 1,
        stageName, true, conv2Opts);
    const conv3 = this.buildConv_(conv2,
        parseInt(nameIndex) + 2, stageName, false, {});
    if (downsample) {
      const resOpts = {};
      if (stride > 1) {
        resOpts.strides = [stride, stride];
      }
      residual = this.buildConv_(
          input, parseInt(nameIndex) + 3, stageName, false, resOpts);
    }
