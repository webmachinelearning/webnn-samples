...
export class Lenet {
  ...
  async load() {
    ...
    const conv1Filter = nn.constant(
        {type: 'tensor-float32', dimensions: conv1FitlerShape},
        conv1FilterData);
    byteOffset +=
    ...
    const add1Bias = nn.constant(
        {type: 'tensor-float32', dimensions: add1BiasShape}, add1BiasData);
    ...
    const pool1 =
        nn.maxPool2d(add1, pool1WindowShape, [0, 0, 0, 0], pool1Strides);

    const conv2FilterShape = [50, 20, 5, 5];
    const conv2Filter = nn.constant(
        {type: 'tensor-float32', dimensions: conv2FilterShape},
        new Float32Array(
    ...
    const add2Bias = nn.constant(
        {type: 'tensor-float32', dimensions: add2BiasShape},
    ...
    const pool2 =
        nn.maxPool2d(add2, pool2WindowShape, [0, 0, 0, 0], pool2Strides);
    ...
    const matmul1Weights = nn.constant(
        {type: 'tensor-float32', dimensions: matmul1Shape},
    ...
    const add3Bias = nn.constant(
        {type: 'tensor-float32', dimensions: add3BiasShape},
    ...
    const matmul2Weights = nn.constant(
        {type: 'tensor-float32', dimensions: matmul2Shape},
    ...
    const add4Bias = nn.constant(
        {type: 'tensor-float32', dimensions: add4BiasShape},
