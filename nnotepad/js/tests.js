...
async function test(expr, expected) {
  ...
  try {
    const [builderFunc, src] = NNotepad.makeBuilderFunction(expr);
...
document.addEventListener('DOMContentLoaded', async e => {
  ...
  await test('12.34f32', {dataType: 'float32', shape: [], buffer: [Math.fround(12.34)]});
  ...
  await test('[12.34]f32', {dataType: 'float32', shape: [1], buffer: [Math.fround(12.34)]});
  await test('[12.34]f16', {dataType: 'float16', shape: [1], buffer: [12.34375]});

  Harness.section('Tensors');
  await test(`A = [[1,7],[2,4]]  B = [[3,3],[5,2]]  matmul(A,B)`, {dataType: 'float32', shape: [2,2], buffer: [38,17,26,14]});
  await test(`M = [[2,8,3],[5,4,1]]  N = [[4,1],[6,3],[2,4]]  matmul(M,N)`, {dataType: 'float32', shape: [2,2], buffer: [62,38,46,21]});

  Harness.section('Dictionaries');
  await test('linear(10, {})', {dataType: 'float32', shape: [], buffer: [10]});
  await test('linear(10, {alpha: 2, beta: 3})', {dataType: 'float32', shape: [], buffer: [23]});

  Harness.section('String arguments');
  await test(`cast([1,2,3], 'int8')`, {dataType: 'int8', shape: [3], buffer: [1,2,3]});
  await test(`cast([1,2,3], "int8")`, {dataType: 'int8', shape: [3], buffer: [1,2,3]});

  Harness.section('Multiple output tensors');
  await test(`split([1,2,3,4], 2)`, [{dataType: 'float32', shape: [2], buffer: [1,2]},{dataType: 'float32', shape: [2], buffer: [3,4]}]);

  Harness.section('Multiple input tensors');
  await test(`A = [1,2]  B = [3,4]  concat([A,B], 0)`, {dataType: 'float32', shape: [4], buffer: [1,2,3,4]});
  await test(`concat([identity([1,2]),identity([3,4])], 0)`, {dataType: 'float32', shape: [4], buffer: [1,2,3,4]});

  Harness.section('Regression tests');
  await test(`concat([[1,2],[3,4]], 0)`, {dataType: 'float32', shape: [4], buffer: [1,2,3,4]});
  //await test(`input = [[[1,2],[3,4]],[[5,6],[7,8]]]  weight = [[[1,2],[1,2],[1,2],[1,2]]]  rweight = [[[1],[1],[1],[1]]]  lstm(input, weight, rweight, 2, 1)`, {});
